...
function SectionCard({ icon, title, subtitle, action, children }) {
  return (
    <div style={{ backgroundColor: "#FFFFFF", border: "1px solid #E5E2D8", borderRadius: "20px", boxShadow: "0 4px 20px rgba(0,0,0,0.05)", overflow: "hidden", marginBottom: "36px", animation: "fadeIn 0.25s ease" }}>
      <div style={{ padding: "18px 36px", borderBottom: "1px solid #F0EDE6", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "14px", minWidth: 0 }}>
          ...
          <div>
            ...
            {subtitle && <p style={{ fontSize: "14px", color: "#9CA3AF", margin: "2px 0 0" }}>{subtitle}</p>}
          </div>
        </div>
        {action && <div style={{ flexShrink: 0 }}>{action}</div>}
      </div>
      <div style={{ padding: "20px 36px 22px" }}>{children}</div>
    </div>
  );
}

function FieldLabel({ children }) {
  return <label style={{ display: "block", fontSize: "14px", fontWeight: 700, color: "#374151", marginBottom: "6px", letterSpacing: "0.1px" }}>{children}</label>;
}
    ...
    <div>
      ...
      <div style={{ position: "relative" }}>
        <input
          ...
          style={{
            width: "100%", padding: "11px 18px", paddingRight: suffix ? "50px" : "18px",
...
function ProfileSection({ user, updateUser }) {
  ...
  return (
    <SectionCard
      ...
      action={<PrimaryButton onClick={save} loading={loading} saved={saved}>💾 Save Profile</PrimaryButton>}
    >
      {/* Avatar */}
      <div style={{ display: "flex", alignItems: "center", gap: "14px", padding: "12px 18px", backgroundColor: "#F8F6F0", borderRadius: "14px", marginBottom: "18px" }}>
        <div style={{ width: "52px", height: "52px", background: "linear-gradient(135deg,var(--primary),var(--primary-light))", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", fontWeight: 800, color: "#FFFFFF", flexShrink: 0 }}>{initials}</div>
        <div>
          <p style={{ fontSize: "16px", fontWeight: 800, color: "#111827", margin: 0 }}>{form.name || "Your Name"}</p>
          <p style={{ fontSize: "13px", color: "#9CA3AF", margin: "2px 0 0 0" }}>{form.email || "your@email.com"}</p>
        </div>
      </div>
...
function PasswordSection() {
  ...
  return (
    <SectionCard
      ...
      action={<PrimaryButton onClick={save} loading={loading} saved={saved} disabled={!canSave}>🔑 Update Password</PrimaryButton>}
    >
...
function AIProviderSection({ user, updateUser }) {
  ...
  return (
    <SectionCard
      icon="🧠"
      title="AI Provider"
      subtitle="Choose the language model that powers your research"
      action={<PrimaryButton onClick={save} loading={loading} saved={saved}>🧠 Save Provider</PrimaryButton>}
    >
...
function AppearanceSection({ updateUser }) {
  ...
  return (
    <SectionCard
      icon="🎨"
      title="Appearance"
      subtitle="Customize the visual theme of your workspace"
      action={<PrimaryButton onClick={apply} loading={saving} saved={saved} disabled={pending === theme}>🎨 Apply Theme</PrimaryButton>}
    >
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", marginBottom: 0 }}>
        {THEMES.map(t => {
...
export default function Settings() {
  ...
  // Keep the original full width and left alignment
  const contentStyle = {
    width: "100%",
    maxWidth: "100%",
    margin: "0",
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{
                ...
                flex: "1 1 0",
                padding: t.id === "profile" && on ? "13px 16px" : "11px 16px",
                fontSize: t.id === "profile" && on ? "16px" : "15px",
